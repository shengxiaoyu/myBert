...
class EventProcesss(DataProcessor):
    ...
    def __get_file__(self,data_dir,mode):
        ...
        eventSentences = list(map(lambda x: (x, 1), eventSentences))
        noiseSentences = list(map(lambda x: (x, 0), noiseSentences))

        firstSentences = []
        firstSentences.extend(eventSentences)
        firstSentences.extend(eventSentences)
        firstSentences.extend(eventSentences)
        random.shuffle(firstSentences)

        secondSentences = noiseSentences
        secondSentences.extend(eventSentences)
        random.shuffle(secondSentences)

        examples = []
        flag = True
        index = 0
        for firstSentence, secondSentence in zip(firstSentences, secondSentences):
            guid = "%s-%d" % (mode,index)
            if (firstSentence[1] == 1 and secondSentence[1] == 1):
                input = None
                if (flag):
                    input = InputExample(guid=guid, text_a=tokenization.convert_to_unicode(firstSentence[0]),
                                         text_b=tokenization.convert_to_unicode(secondSentence[0]), label=True)
                else:
                    input = InputExample(guid=guid, text_a=tokenization.convert_to_unicode(secondSentence[0]),
                                         text_b=tokenization.convert_to_unicode(firstSentence[0]), label=True)
                examples.append(input)
            else:
                input = None
                if (flag):
                    input = InputExample(guid=guid, text_a=tokenization.convert_to_unicode(firstSentence[0]),
                                         text_b=tokenization.convert_to_unicode(secondSentence[0]), label=False)
                else:
                    input = InputExample(guid=guid, text_a=tokenization.convert_to_unicode(secondSentence[0]),
                                         text_b=tokenization.convert_to_unicode(firstSentence[0]), label=False)
                examples.append(input)
            flag = not flag
            index += 1
        return examples
